...
void DrawOneVar(const char* var, vector<string> list_name, vector<TFile*> list_file, const int i_ds0=-1)
{
  ...
  vector<TH1*> list_h1;
  THStack hs("hs", Form(";%s;Acceptance", var));
  TLegend leg(0.90, 0.90-n_ds*0.05, 0.99, 0.90);
  
  for (int i_ds = 0; i_ds < n_ds; i_ds++) {
    TH1* h1 = (TH1*)list_file[i_ds]->Get( Form("h1_acc_%s", var) );
    if (! h1) {
  ...
  hs.Draw("nostack");
  leg.Draw();
  c1->SaveAs( Form("result/comp-acc/h1_acc_%s.pdf", var) );
  ...
  if (i_ds0 >= 0) {
    ...
    for (int i_ds = 0; i_ds < n_ds; i_ds++) {
      ...
      c1->cd();
      if (is_1st) {
        h1_ratio->SetTitle( Form(";%s;Acceptance Ratio", var) );
        h1_ratio->GetYaxis()->SetRangeUser(0.7, 1.2);
        h1_ratio->Draw("E1");
        is_1st = false;
      } else {
        h1_ratio->Draw("E1same");
      }
      leg2.AddEntry(h1_ratio, list_name[i_ds].c_str(), "pl");

      c2->cd();
      h1_ratio2->SetTitle( Form("%s / %s;%s;Acceptance Ratio", name, name0, var) );
      h1_ratio2->GetYaxis()->SetRangeUser(0.7, 1.2);
      ...
      oss.str("");
      oss << "p1 = (" << 1e3 * f1->GetParameter(1) << " #pm " << 1e3 * f1->GetParError(1) << ") #times 10^{-3}";
      tex.DrawLatex(0.12, 0.75, oss.str().c_str());
      c2->SaveAs( Form("result/comp-acc/h1_ratio_%s_%s.pdf", var, name) );
    }
    c1->cd();
    leg2.Draw();
    c1->SaveAs( Form("result/comp-acc/h1_ratio_%s.pdf", var) );
  }
...
void comp_acc_datasets()
{
  gStyle->SetOptStat(0);
  //gStyle->SetOptFit(true);
  gSystem->mkdir("result/comp-acc", true);

  vector<string> list_name = { "rs57", "rs59", "rs62", "rs67", "rs70" };
  vector<TFile*> list_file = {
    new TFile("0828/lh2/rs57/acc/result.root"), 
    new TFile("0828/lh2/rs59/acc/result.root"), 
    new TFile("0828/lh2/rs62/acc/result.root"), 
    new TFile("0828/lh2/rs67/acc/result.root"), 
    new TFile("0828/lh2/rs70/acc/result.root"), 
  };

  const int i_ds0 = 3; // Used as the denominator of the ratio plots.
  DrawOneVar("mass", list_name, list_file, i_ds0);
  DrawOneVar("xF"  , list_name, list_file, i_ds0);
  DrawOneVar("x1"  , list_name, list_file, i_ds0);
  DrawOneVar("x2"  , list_name, list_file, i_ds0);
  DrawOneVar("pT"  , list_name, list_file, i_ds0);
